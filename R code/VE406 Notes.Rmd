...
```{r}
oc.df = read.table("./in-class demo/Data/old_car.txt")
# oc.LM = lm(price~age, data = oc.df)
```
# L06 Prediction 
...
library(xlsx)
trans.df = read.xlsx("./in-class demo/Data/transistor.xlsx", sheetIndex = 1)
...
```{r}
sim.df = read.csv("./in-class demo/Data/sim_poly_class.csv.bz2")
str(sim.df)
...
```{r}
chem_pro.df = read.table("./in-class demo/Data/chem_pro.csv", sep = ",", header = TRUE)
# check 
...
Heteroskedasticity
```{r}
cleaning.df = read.table("./in-class demo/Data/cleaning.txt", header = TRUE)
summary(cleaning.df)
...
```{r}
cvrv.df = read.table("./in-class demo/Data/cvrv.csv", sep = ",", header = TRUE)
...
```{r}
evap.df = read.table("./in-class demo/Data/evap.txt", header = TRUE)
...
plot.new
quartz()
puromycin.df = read.table("./in-class demo/Data/puromycin.txt", header = TRUE)
str(puromycin.df)
...
```{r}
credit.df = read.table("./in-class demo/Data/credit.csv", header = TRUE, sep = ",")
str(credit.df)
# visualize the data
# Scatter plot
...
ingots.df = credit.df = read.table("./in-class demo/Data/ingots.csv", header = TRUE, sep = ",")
...
```{r}
bomber.df = read.table("./in-class demo/Data/aircraft.csv", header = TRUE, sep = ",")
str(bomber.df)
# the MLE of beta 
...
semiconductor.df = read.table("./in-class demo/Data/semiconductor.txt", colClasses = c(rep("factor", 7), "numeric"), header = TRUE)
...
cmb.df = read.table("./in-class demo/Data/cmb.csv", sep=',', header = TRUE)
library(splines)
n.samp = 10  # try different size spline with n
index = sample(nrow(cmb.df), n.samp) # 随机选取十个
cmb_train = cmb.df[index, ]
cmb_train = cmb_train[order(cmb_train$freq), ]
ispl = interpSpline(cmb_train$freq, cmb_train$power)
pred.x = seq(min(cmb_train$freq), max(cmb_train$freq), length.out = 200) 
...
plot(power~freq, data = cmb.df, col = "deepskyblue", main ="Cosmic Microwave Backgroud Radiation", sub = "Gaussian knernel regression fit")
lines(density(x = cmb.df$power, bw = 0.5, kernel = "gaussian"), col = 3)
```
...
plot(power~freq, data = cmb.df, xlim=c(0,100), ylim = c(0, 3000), col = "deepskyblue", main ="Cosmic Microwave Backgroud Radiation", sub = "Boundary bias of kernel regression")
...
y = m + 0.15 * rnorm(n)
plot(x, sqrt(x*(1-x)) * sin(2.1*pi/(x+0.05)))
# 这里的图怎么画的？？
```
```{r}
require(graphics)
...
#L21Generalised Additive Model 
Additive models that use splines 
```{r}
...
summary(pre.LM)
plot(pre.LM, which = 1) # non-linearity issue 
plot(pre.LM, which = 2) # the normality seems to be ok 
plot(pre.LM, which = 3) 
plot(pre.LM, which = 4) 
plot(pre.LM, which = 5) 
plot(pre.LM, which = 6) 
acf(pre.LM$residuals) # the errors might lack independence 

# try the linearity 
# bs: B-Spline Basis for polynomial Splines 
pre.spline.LM = lm(prestige~bs(income, knots = c(9000))+bs(education, knots = c(9, 12)))
# the choices of knots are made largely based on residual plots and the knowledge regarding education system
# add one knot for income and two knots for education
plot(pre.spline.LM, which = 1)  # seems to fix the non-linearity problem 
...
# splitting the data and investigate them indiviadaully 
...
```{r}
library(ISLR);
wage.df = Wage[, c("year", "age", "education", "wage")]
str(wage.df)

# smoothing spline 
# install.packages("gam")
library(gam)
wage.inv.GAM = gam(wage~s(age)+s(year)+education, family = Gamma(link = "inverse"), data = wage.df)
wage.log.GAM = gam(wage~s(age)+s(year)+education, family = Gamma(link = "log"), data = wage.df)

# inverse link residual 
res.inv.df = data.frame(cond_m = fitted(wage.inv.GAM), pear_r = residuals(wage.inv.GAM, type = "pearson")) 
with(res.inv.df, plot( cond_m, pear_r, main = "Residual Plot", cex.lab = 1.5, cex.main = 2)) + with(res.inv.df, lines(smooth.spline( cond_m, pear_r), col = "red", lty = 2, lwd = 2)) + mtext("wage.inv.GAM")

# log link residual 
res.log.df = data.frame(cond_m = fitted(wage.log.GAM), pear_r = residuals(wage.log.GAM, type = "pearson")) 
with(res.log.df, plot( cond_m, pear_r, main = "Residual Plot", cex.lab = 1.5, cex.main = 2)) + with(res.log.df, lines(smooth.spline( cond_m, pear_r), col = "red", lty = 2, lwd = 2)) + mtext("wage.log.GAM")

## two plots are similar, expects similar results from both
...
wage.normal.GAM = wage.LM # wrong normal.GAM 
AIC(wage.normal.GAM, wage.inv.GAM, wage.log.GAM) # inverse better 
...
for (i in 1:k){
  ...
  wage.inv.GAM = gam(wage~s(age)+s(year)+education, data = wage.df[-index,]) 
  pred.inv[, i] = predict(wage.inv.GAM, wage.df[index,], type = "response")
}

for (i in 1:k){
  start = (i-1)*n.test+1; end = i*n.test
  index = row.index[start:end]
  wage.log.GAM = gam(wage~s(age)+s(year)+education, family = Gamma(link = "log"), data = wage.df[-index,]) 
  pred.log[, i] = predict(wage.log.GAM, wage.df[index,], type = "response")
}
...
```{r}
# logistic regression under GAM to see whether collectively age, year, education could explain the apparent separation in wage 
wage.LG.GAM = gam(I(wage > 250)~s(age)+s(year)+education, data = Wage, family = binomial)
1 - pchisq(wage.LG.GAM$deviance, wage.LG.GAM$df.residual) # no indication of lack of fit 

# so! might need mixture model ! 
```

#L22 PCA
PCA influenced by the magnitude of each variable (the results obtained when we perform PCA will also depend on whether the variables have been individually scaled)
```{r}
...
biplot(USArrests.PCA, main = "Scaled", xlab = "First Principal Component", ylab = "Second Principal Component", asp = TRUE, cex.lab = 1.5, cex.main = 2, cex=c(3, 1.2), xlim = c(-0.3, 0.3), ylim = c(-0.3, 0.35), col=c("deepskyblue", "chocolate"), xlabs=rep("ů", nrow(USArrests))) 
...
phi
# the first principal component (PC1) roughly corresponds to an overall rate of serious crimes since Murder, Assault, and Rape have the largest values. The second component (PC2) is affected by UrbanPop more than the other three variables, so it roughly corresponds to the level of urbanization of the state, with some opposite, smaller influence by murder rate.
```
# L23 Factor Analysis 
